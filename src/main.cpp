...
#if defined(NDEBUG)
# error "Nosferatu cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x15ce684d89db69debfe2f8df03fb07db7f1d0cbacf29805ee9dd18995032123c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nosferatu: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nosferatu Signed Message:\n";
...
{
    // Nosferatu: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nosferatu: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nosferatu: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x15ce684d89db69debfe2f8df03fb07db7f1d0cbacf29805ee9dd18995032123c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1431441124, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "12/May/2015 swee";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xe8ce5974ef99d8fed669f4b078f50a87c50492266f8163256a48c2c15382f26f"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Nosferatu: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NosferatuMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NosferatuMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NosferatuMiner(CWallet *pwallet)
{
    printf("NosferatuMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nosferatu-miner");
    ...
    try { loop {
        ...
        printf("Running NosferatuMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NosferatuMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NosferatuMiner, pwallet));
}
